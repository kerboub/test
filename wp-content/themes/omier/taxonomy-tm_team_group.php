...
get_header(); ?>
	...
	<section id="primary" class="content-area <?php echo themetechmount_sanitize_html_classes(themetechmount_sidebar_class('content-area')); ?>">
		...
		<?php if ( have_posts() ) : ?>
			...
			$view   = ( !empty($boldman_theme_options['teamcat_view']) ) ? $boldman_theme_options['teamcat_view'] : 'top-image' ;
			$column = ( !empty($boldman_theme_options['teamcat_column']) ) ? $boldman_theme_options['teamcat_column'] : 'three' ;			
			?>	
