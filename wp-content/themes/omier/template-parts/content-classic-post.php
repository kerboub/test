...
<article id="post-<?php the_ID(); ?>" <?php post_class( themetechmount_sanitize_html_classes(themetechmount_postlayout_class()) ); ?> >
	...
	<?php if( 'quote' != get_post_format() ) : ?>
		<div class="entry-content">
			...
			the_content( sprintf(
				esc_attr__( 'Read More %s', 'boldman' ),
	...
	if( ( !empty($prev_post) || !empty($next_post) ) && shortcode_exists('tm-btn') ){
		...
		<div class="tm-post-prev-next-buttons clearfix">
			<?php
			if( !empty( $prev_post ) ){
				echo do_shortcode('[tm-btn title="' . esc_attr__('Previous', 'boldman') . '" style="flat" shape="square" color="skincolor" size="sm" i_align="left" i_icon_themify="tm-boldman-icon-angle-double-left" add_icon="true" link="url:' . urlencode(esc_url( get_permalink( $prev_post->ID ) )) . '|title:' . rawurlencode($prev_post->post_title) . '||" el_class="tm-left-align-btn"]');
			};
			// Next post
			if ( !empty($next_post) ){
				echo do_shortcode('[tm-btn title="' . esc_attr__('Next', 'boldman') . '" style="flat" shape="square" color="skincolor" size="sm" i_align="right" i_icon_themify="tm-boldman-icon-angle-double-right" add_icon="true" link="url:' . urlencode(esc_url( get_permalink( $next_post->ID ) )) . '|title:' . rawurlencode($next_post->post_title) . '||" el_class="tm-right-align-btn"]');
			};
			?>
		</div>
		<?php
