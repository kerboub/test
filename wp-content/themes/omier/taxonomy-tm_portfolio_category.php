...
get_header(); ?>
	...
	<section id="primary" class="content-area <?php echo themetechmount_sanitize_html_classes(themetechmount_sidebar_class('content-area')); ?>">
		...
		<?php if ( have_posts() ) : ?>
			...
			$view   = ( !empty($boldman_theme_options['pfcat_view']) ) ? $boldman_theme_options['pfcat_view'] : 'overlay' ;
			$column = ( !empty($boldman_theme_options['pfcat_column']) ) ? $boldman_theme_options['pfcat_column'] : 'three' ;			
			?>	
