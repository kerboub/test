...
function themetechmount_header_style_change_triggers(){
	...
	var topbar_right_text			  = '<ul class="top-contact"><li><i class="fa fa-envelope-o"></i> <strong>Email: </strong><a href="mailto:info@example.com.com">info@example.com</a></li></ul>[tm-social-links tooltip="no"]';
	var topbar_left_text			  = '<ul class="top-contact tm-highlight-left"><li><i class="fa fa-phone"></i><strong>Client Services:</strong> 0 (143) 456 7897</li></ul>';
	var titlebar_height				  = '283';
	var mainmenufont_color            = '#182333';
	var topbar_bg_color				  = 'darkgrey';
	var header_height                 = '115';
	var logoimg						  = 'logo.png';
	var logo_max_height				  = '40';
	var mainmenu_active_link_color    = 'skin';
	var stickymainmenufontcolor_color = '#182333';
	var sticky_header_bg_color        = 'white';
	var dropmenu_background_color     = '#ffffff';
	var dropdownmenufont_color        = '#182333';
	var header_text      			  = '[tm-btn title="REQUEST QUOTE" style="outline" shape="square" color="black" link="url:%23|||"]';
	var infostack_column_one		  = '<div class="header-icon"><div class="icon"><i class="themifyicon ti-mobile"></i></div></div><div class="header-content"><h3>Email</h3><h5>info@example.com</h5></div>';
	var infostack_column_two		  = '<div class="header-icon"><div class="icon"><i class="themifyicon ti-comment-alt"></i></div></div><div class="header-content"><h3>Address</h3><h5>24 Fifth st, Los Angeles, USA</h5></div>';
	var infostack_column_three		  = '<div class="header-icon"><div class="icon"><i class="themifyicon ti-location-pin"></i></div></div><div class="header-content"><h3>Address</h3><h5> 24 Fifth st, Los Angeles, USA</h5></div>';
	var classic_infostack_column_one  = '<div class="header-icon"><div class="icon"><i class="fa fa-map-marker"></i></div></div><div class="header-content"><h3>Address</h3><h5>24 Fifth st, Los Angeles, USA</h5></div>';
	var classic_infostack_column_two  = '<div class="header-icon"><div class="icon"><i class="fa fa-user"></i></div></div><div class="header-content"><h3>Guaranteed Work</h3><h5>Qualified Staff</h5></div>';
	var classic_infostack_column_three  = '<div class="header-icon"><div class="icon"><i class="fa fa-truck"></i></div></div><div class="header-content"><h3>24/7 Emergency Service</h3><h5>Monday - Sunday</h5></div>';
	
	jQuery('input[name="boldman_theme_options[headerstyle]"]').change(function() {
		...
		if (this.value == 'classic') { // Classic
			jQuery('select[name="boldman_theme_options[header_bg_color]"]').val(header_bg_color).change();
			jQuery('input[name="boldman_theme_options[titlebar_background][color]"]').iris('color', titlebar_background_color);  // 
			jQuery('select[name="boldman_theme_options[titlebar_text_color]"]').val(titlebar_text_color);
			jQuery('textarea[name="boldman_theme_options[topbar_right_text]"]').val(topbar_right_text);  //
			jQuery('input[name="boldman_theme_options[titlebar_height]"').val(titlebar_height);  // Titlebar height
			jQuery('input[name="boldman_theme_options[show_topbar]"]').prop('checked', true);  // Show topbar
			jQuery('input[name="boldman_theme_options[header_search]"]').prop('checked', true);  // Show header search
			jQuery('input[name="boldman_theme_options[mainmenufont][color]"').iris('color', mainmenufont_color);  // 
			jQuery('select[name="boldman_theme_options[topbar_bg_color]"]').val('darkgrey').change();
			jQuery('input[name="boldman_theme_options[header_height]"]').val(header_height);  // Header Height
			jQuery('input[name="boldman_theme_options[logoimg][id]"]').val('');
			jQuery('input[name="boldman_theme_options[logoimg][thumb-url]"]').val( imgurl+logoimg );
			jQuery('input[name="boldman_theme_options[logoimg][full-url]').val( imgurl+logoimg );
			jQuery('input[name="boldman_theme_options[logo_max_height]"]').val(logo_max_height);  //
			jQuery('select[name="boldman_theme_options[mainmenu_active_link_color]"]').val(mainmenu_active_link_color).change(); // Select
			jQuery('input[name="boldman_theme_options[stickymainmenufontcolor]"]').iris('color', stickymainmenufontcolor_color);  // 
			jQuery('select[name="boldman_theme_options[sticky_header_bg_color]"]').val(sticky_header_bg_color).change();		
			jQuery('input[name="boldman_theme_options[dropmenu_background][color]"]').iris('color', dropmenu_background_color);  // Dropdown Menu color
			jQuery('input[name="boldman_theme_options[dropdownmenufont][color]"]').iris('color', dropdownmenufont_color);  // Dropdown Menu color
			jQuery('textarea[name="boldman_theme_options[header_text]"]').val(header_text);  //				
		} else if ( this.value == 'classic-overlay' || this.value == 'overlay' ) {  // Overlay
			jQuery('select[name="boldman_theme_options[header_bg_color]"]').val('darkgrey').change();
			jQuery('input[name="boldman_theme_options[titlebar_background][color]"]').iris('color', titlebar_background_color);  // 
			jQuery('select[name="boldman_theme_options[titlebar_text_color]"]').val(titlebar_text_color);
			jQuery('input[name="boldman_theme_options[show_topbar]"]').prop('checked', true);  // Show topbar
			jQuery('input[name="boldman_theme_options[titlebar_height]"').val('450');  // Titlebar height
			jQuery('input[name="boldman_theme_options[header_search]"]').prop('checked', true);  // Show header search
			jQuery('input[name="boldman_theme_options[mainmenufont][color]"').iris('color', '#ffffff');   // 
			jQuery('select[name="boldman_theme_options[topbar_bg_color]"]').val(topbar_bg_color).change();
			jQuery('input[name="boldman_theme_options[header_height]"]').val('99');  // Header Height
			jQuery('input[name="boldman_theme_options[logoimg][id]"]').val('');
			jQuery('input[name="boldman_theme_options[logoimg][thumb-url]"]').val( imgurl+'logo-white.png' );
			jQuery('input[name="boldman_theme_options[logoimg][full-url]').val( imgurl+'logo-white.png' );
			jQuery('input[name="boldman_theme_options[logo_max_height]"]').val(logo_max_height);  //
			jQuery('select[name="boldman_theme_options[mainmenu_active_link_color]"]').val(mainmenu_active_link_color).change(); // Select
			jQuery('input[name="boldman_theme_options[stickymainmenufontcolor]"]').iris('color', '#ffffff');// 
			jQuery('select[name="boldman_theme_options[sticky_header_bg_color]"]').val('darkgrey').change();
			jQuery('input[name="boldman_theme_options[dropmenu_background][color]"]').iris('color', dropmenu_background_color); // Dropdown Menu color
			jQuery('input[name="boldman_theme_options[dropdownmenufont][color]"]').iris('color', dropdownmenufont_color);  
			jQuery('textarea[name="boldman_theme_options[header_text]"]').val('[tm-btn title="REQUEST QUOTE" style="outline" shape="square" color="skincolor" link="url:%23|||"]');  //
		} else if (this.value == 'infostack') {
			jQuery('select[name="boldman_theme_options[header_bg_color]"]').val(header_bg_color).change();
			jQuery('input[name="boldman_theme_options[titlebar_background][color]"]').iris('color', titlebar_background_color);  // 
			jQuery('select[name="boldman_theme_options[titlebar_text_color]"]').val(titlebar_text_color);
			jQuery('textarea[name="boldman_theme_options[topbar_left_text]"]').val('<ul class="top-contact"><li>Your Trusted 24 Hours Service Provider!</li></ul>'); 
			jQuery('textarea[name="boldman_theme_options[topbar_right_text]"]').val('[tm-social-links tooltip="no"]<ul class="top-contact tm-highlight-right"><li><strong><i class="fa fa-phone"></i>Talk To Expert :</strong> <span class="tel-no">0 (143) 456 7897</span></li></ul>'); //
			jQuery('input[name="boldman_theme_options[titlebar_height]"').val('417');  // Titlebar height
			jQuery('input[name="boldman_theme_options[show_topbar]"]').prop('checked', true);  // Show topbar
			jQuery('input[name="boldman_theme_options[header_search]"]').prop('checked', true);  // Show header search
			jQuery('input[name="boldman_theme_options[mainmenufont][color]"').iris('color', mainmenufont_color);  // 
			jQuery('select[name="boldman_theme_options[topbar_bg_color]"]').val('darkgrey').change();
			jQuery('input[name="boldman_theme_options[header_height]"]').val('156');  // Header Height
			jQuery('input[name="boldman_theme_options[logoimg][id]"]').val('');
			jQuery('input[name="boldman_theme_options[logoimg][thumb-url]"]').val( imgurl+'logo.png' );
			jQuery('input[name="boldman_theme_options[logoimg][full-url]').val( imgurl+'logo.png' );
			jQuery('input[name="boldman_theme_options[logo_max_height]"]').val('45');   //  //
			jQuery('select[name="boldman_theme_options[mainmenu_active_link_color]"]').val(mainmenu_active_link_color).change(); // Select
			jQuery('input[name="boldman_theme_options[stickymainmenufontcolor]"]').iris('color', stickymainmenufontcolor_color);  // 
			jQuery('select[name="boldman_theme_options[sticky_header_bg_color]"]').val(sticky_header_bg_color).change();
			jQuery('select[name="boldman_theme_options[infostack_phone_text]"]').val('').change();
			jQuery('textarea[name="boldman_theme_options[infostack_column_one]"]').val(infostack_column_one);
			jQuery('textarea[name="boldman_theme_options[infostack_column_two]"]').val(infostack_column_two);
			jQuery('textarea[name="boldman_theme_options[infostack_column_three]"]').val(infostack_column_three);			
			jQuery('input[name="boldman_theme_options[dropmenu_background][color]"]').iris('color', dropmenu_background_color);  // Dropdown Menu color
			jQuery('input[name="boldman_theme_options[dropdownmenufont][color]"]').iris('color', dropdownmenufont_color);  // Dropdown Menu color			
		} else if (this.value == 'classicinfo') {
			jQuery('select[name="boldman_theme_options[header_bg_color]"]').val(header_bg_color).change();
			jQuery('input[name="boldman_theme_options[show_topbar]"]').prop('checked', true);  // Show topbar
			jQuery('input[name="boldman_theme_options[header_search]"]').prop('checked', true);  // Show header search
			jQuery('select[name="boldman_theme_options[topbar_bg_color]"]').val('darkgrey').change();
			jQuery('input[name="boldman_theme_options[header_height]"]').val('115');  // Header Height
			jQuery('input[name="boldman_theme_options[logo_max_height]"]').val('40');   //
			jQuery('textarea[name="boldman_theme_options[topbar_left_text]"]').val('<ul class="top-contact tm-highlight-left"><li><i class="fa fa-phone"></i><strong>Client Services:</strong> 0 (143) 456 7897</li></ul>'); 
			jQuery('textarea[name="boldman_theme_options[topbar_right_text]"]').val('<ul class="top-contact"><li><i class="fa fa-envelope-o"></i> <strong>Email: </strong><a href="mailto:info@example.com.com">info@example.com</a></li></ul>[tm-social-links tooltip="no"]');  //
			jQuery('input[name="boldman_theme_options[mainmenufont][color]"').iris('color', mainmenufont_color);  // 
			jQuery('select[name="boldman_theme_options[mainmenu_active_link_color]"]').val(mainmenu_active_link_color).change();
			jQuery('input[name="boldman_theme_options[titlebar_height]"').val('283'); // Titlebar height
			jQuery('input[name="boldman_theme_options[stickymainmenufontcolor]"').iris('color', stickymainmenufontcolor_color);  // 
			jQuery('input[name="boldman_theme_options[logoimg][id]"]').val('');
			jQuery('input[name="boldman_theme_options[logoimg][thumb-url]"]').val( imgurl+'logo.png' );
			jQuery('input[name="boldman_theme_options[logoimg][full-url]').val( imgurl+'logo.png' );		
			jQuery('input[name="boldman_theme_options[dropmenu_background][color]"]').iris('color', '#ffffff');  // Dropdown Menu color
			jQuery('input[name="boldman_theme_options[dropdownmenufont][color]"]').iris('color', dropdownmenufont_color);   // Dropdown Menu color
			jQuery('select[name="boldman_theme_options[sticky_header_bg_color]"]').val(sticky_header_bg_color).change();
			jQuery('textarea[name="boldman_theme_options[infostack_phone_text]"]').val('');
			jQuery('textarea[name="boldman_theme_options[infostack_column_one]"]').val(classic_infostack_column_one);
			jQuery('textarea[name="boldman_theme_options[infostack_column_two]"]').val(classic_infostack_column_two);
			jQuery('textarea[name="boldman_theme_options[infostack_column_three]"]').val(classic_infostack_column_three);	
		} else if (this.value == 'toplogo') {
			jQuery('select[name="boldman_theme_options[header_bg_color]"]').val(header_bg_color).change();
			jQuery('input[name="boldman_theme_options[show_topbar]"]').prop('checked', true);  // Show topbar
			jQuery('input[name="boldman_theme_options[header_search]"]').prop('checked', false);  // Show header search
			jQuery('select[name="boldman_theme_options[topbar_bg_color]"]').val('skincolor').change();
			jQuery('input[name="boldman_theme_options[header_height]"]').val('150');  // Header Height
			jQuery('input[name="boldman_theme_options[logo_max_height]"]').val('45');   //
			jQuery('textarea[name="boldman_theme_options[topbar_left_text]"]').val('<ul class="top-contact"><li>Your Trusted 24 Hours Service Provider!</li></ul>'); 
			jQuery('textarea[name="boldman_theme_options[topbar_right_text]"]').val('<ul class="top-contact"><li><i class="fa fa-envelope-o"></i> <strong>Email: </strong><a href="mailto:info@example.com.com">info@example.com</a></li></ul>[tm-social-links tooltip="no"]');  //
			jQuery('input[name="boldman_theme_options[titlebar_height]"').val('283'); // Titlebar height
			...
			jQuery('input[name="boldman_theme_options[dropdownmenufont][color]"]').iris('color', dropdownmenufont_color);   // Dropdown Menu color
			jQuery('select[name="boldman_theme_options[sticky_header_bg_color]"]').val(sticky_header_bg_color).change();
		} else if (this.value == 'centerlogo-overlay') {
			jQuery('select[name="boldman_theme_options[header_bg_color]"]').val('darkgrey').change();
			jQuery('input[name="boldman_theme_options[show_topbar]"]').prop('checked', true);  // Show topbar
			jQuery('input[name="boldman_theme_options[header_search]"]').prop('checked', true);  // Show header search
			jQuery('select[name="boldman_theme_options[topbar_bg_color]"]').val('darkgrey').change();
			jQuery('input[name="boldman_theme_options[header_height]"]').val('100');  // Header Height
			jQuery('input[name="boldman_theme_options[logo_max_height]"]').val('45');   //
			jQuery('textarea[name="boldman_theme_options[topbar_left_text]"]').val(topbar_left_text); 
			jQuery('textarea[name="boldman_theme_options[topbar_right_text]"]').val('<ul class="top-contact"><li><i class="fa fa-envelope-o"></i> <strong>Email: </strong><a href="mailto:info@example.com.com">info@example.com</a></li></ul>[tm-social-links tooltip="no"]');  //
			jQuery('input[name="boldman_theme_options[titlebar_height]"').val('340'); // Titlebar height
			jQuery('input[name="boldman_theme_options[logoimg][id]"]').val('');
			jQuery('input[name="boldman_theme_options[logoimg][thumb-url]"]').val( imgurl+'logo-white.png' );
			jQuery('input[name="boldman_theme_options[logoimg][full-url]').val( imgurl+'logo-white.png' );		
			jQuery('input[name="boldman_theme_options[dropmenu_background][color]"]').iris('color', '#ffffff');  // Dropdown Menu color
...
function themetechmount_show_hide_titlebar_textcolor(){
	...
	var $this      = jQuery( 'select[name="boldman_theme_options[titlebar_text_color]"]' );
	var $page_this = jQuery( 'select[name="_themetechmount_metabox_group[titlebar_text_color]"]' );
	
	if( $this.length > 0 ){
		if( jQuery($this).val()=='custom' ){
			jQuery( 'input[name="boldman_theme_options[titlebar_heading_font][color]"]' ).closest('.tm-typography-font-color-w').show();
			jQuery( 'input[name="boldman_theme_options[titlebar_subheading_font][color]"]' ).closest('.tm-typography-font-color-w').show();
			jQuery( 'input[name="boldman_theme_options[titlebar_breadcrumb_font][color]"]' ).closest('.tm-typography-font-color-w').show();
		} else {
			jQuery( 'input[name="boldman_theme_options[titlebar_heading_font][color]"]' ).closest('.tm-typography-font-color-w').hide();
			jQuery( 'input[name="boldman_theme_options[titlebar_subheading_font][color]"]' ).closest('.tm-typography-font-color-w').hide();
			jQuery( 'input[name="boldman_theme_options[titlebar_breadcrumb_font][color]"]' ).closest('.tm-typography-font-color-w').hide();
		}
	}
...
function themetechmount_show_hide_color_picker_background(){
	...
	jQuery(items).each(function(n,val){
		...
		var $dropdown_element   = jQuery( 'select[name="boldman_theme_options['+dropdown_name+']"]' );
		
		// show/hide the color picker on load
		if( $dropdown_element.val()=='custom' ){
			jQuery( 'input[name="boldman_theme_options['+background_name+'][color]"]' ).closest('.tm-background-color-w').show();
		} else {
			jQuery( 'input[name="boldman_theme_options['+background_name+'][color]"]' ).closest('.tm-background-color-w').hide();
		}
...
jQuery(document).ready( function() {
	...
	jQuery( 'select[name="boldman_theme_options[titlebar_text_color]"]' ).change(function() {  // Theme Options
		themetechmount_show_hide_titlebar_textcolor();
	});
	...
	jQuery('input[type=radio][name="boldman_theme_options[loaderimg]"]:checked').closest('label').addClass('tm-pre-loader-option-selected');
	jQuery('input[type=radio][name="boldman_theme_options[loaderimg]"]').change(function() {
		jQuery('input[type=radio][name="boldman_theme_options[loaderimg]"]').closest('label').removeClass('tm-pre-loader-option-selected');
