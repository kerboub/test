 ...
 */
if( !function_exists('themetechmount_megamenu_add_theme_boldman_settings') ){
function themetechmount_megamenu_add_theme_boldman_settings($themes) {
	$themes["boldman_theme_settings"] = array(
		'title' => esc_attr__('Boldman Theme Settings', 'boldman'),
...
add_filter("megamenu_themes", "themetechmount_megamenu_add_theme_boldman_settings");
...
function themetechmount_megamenu_override_default_theme($value) {
  ...
  if ( !isset($value['tm-main-menu']['theme']) ) {
    $value['tm-main-menu']['theme'] = 'boldman_theme_settings'; // change my_custom_theme_key to the ID of your exported theme
  }
...
function themetechmount_mmmenu_breakpoint( $options ){
	...
	if( empty($megamenu_themes) ){
		$megamenu_themes = array (
			'boldman_theme_settings' => 
				array (
				'title' => 'Boldman Theme Settings',
				'arrow_up' => 'dash-f343',
...
function themetechmount_image_sizes(){
	...
	// Portfolio style2 
	add_image_size( 'themetechmount-img-portfoliobox', '600', '700', true );
}
}
...
function themetechmount_single_skin_color(){
	...
	if( $post_id ){
		...
		if( !empty($page_meta['skincolor']) ){
			$boldman_theme_options['skincolor'] = esc_attr($page_meta['skincolor']);
		}
	}
}
}
				...
				if( substr($meta, 0, strlen($prefix) ) == $prefix ){
					...
					if( in_array($meta_name, $all_options) ){
						if( themetechmount_is_json($value[0]) && !is_numeric($value[0]) ){
							...
							$original_val = ( isset($boldman_theme_options[$meta_name]) ) ? $boldman_theme_options[$meta_name] : array() ;
							$final_val    = array_merge( $original_val, $final_val );
						} else {
							// string
							$final_val = $value[0];
						}
						$boldman_theme_options[$meta_name] = $final_val;
					}					
				}
			}  // foreach			
		}  // if		
...
/**
 * Flush out the transients used in {@see boldman_categorized_blog()}.
 *
 * @since Boldman 1.0
 */
if( !function_exists('boldman_category_transient_flusher') ){
function boldman_category_transient_flusher() {
	delete_transient( 'boldman_categories' );
}
}
add_action( 'edit_category', 'boldman_category_transient_flusher' );
add_action( 'save_post',     'boldman_category_transient_flusher' );
...
function themetechmount_add_inline_dynamic_css(){
	...
	// Minify CSS style
	if( !empty( $boldman_theme_options['minify'] ) && esc_attr($boldman_theme_options['minify'])==true ){
		$css = themetechmount_minify_css( $css );
	}
	
	// boldman main style
	wp_add_inline_style( 'boldman-main-style', $css );
...
function themetechmount_single_body_background(){
	...
	if( $post_id ){
		...
		if( isset($single_meta['custom_background_switcher']) && $single_meta['custom_background_switcher']==true && !empty($single_meta['custom_background']) ){
			...
			wp_add_inline_style( 'boldman-last-checkpoint', $css );
		}
...
if ( ! function_exists( 'boldman_excerpt_more' ) && ! is_admin() ) :
/**
 ...
 */
function boldman_excerpt_more( $more ) {
	$link = sprintf( '<a href="%1$s" class="more-link">%2$s</a>',
		esc_url( get_permalink( get_the_ID() ) ),
		/* translators: %s: Name of current post */
		sprintf( esc_attr__( 'Continue reading %s', 'boldman' ), '<span class="screen-reader-text">' . get_the_title( get_the_ID() ) . '</span>' )
		);
	return ' &hellip; ' . $link;
}
add_filter( 'excerpt_more', 'boldman_excerpt_more' );
endif;
...
function themetechmount_body_classes($bodyClass){
	...
	if( isset($boldman_theme_options['titlebar_view']) && 
		($boldman_theme_options['titlebar_view'] == 'default' || $boldman_theme_options['titlebar_view'] == 'allleft' || $boldman_theme_options['titlebar_view'] == 'allright' ) &&
		isset($boldman_theme_options['breadcrumb_on_bottom']) &&
		$boldman_theme_options['breadcrumb_on_bottom'] == true
	){
	...
	if( !empty($boldman_theme_options['headerstyle']) ){
		$bodyClass[] = 'tm-headerstyle-'.esc_attr($boldman_theme_options['headerstyle']);
	}
	
	// check if dark background set for container.
	if( isset($boldman_theme_options['inner_background']['background-color']) && trim($boldman_theme_options['inner_background']['background-color'])!='' && themetechmount_check_dark_color(esc_attr($boldman_theme_options['inner_background']['background-color'])) ){
		$bodyClass[] = 'tm-dark-layout';
	}
	
	// show/hide separator line between links in dropdown menu
	if( isset($boldman_theme_options['dropdown_menu_separator']) && trim($boldman_theme_options['dropdown_menu_separator'])=='0' ){
		$bodyClass[] = 'tm-dropmenu-hide-sepline';
	}
	

	// Sticky Fotoer ON/OFF
	if( isset($boldman_theme_options['stickyfooter']) && $boldman_theme_options['stickyfooter']==true ){
		$bodyClass[] = 'themetechmount-sticky-footer';
	}
	
	// Single Portfolio
	if( is_singular('portfolio') ){
		$portfolioView        = esc_attr($boldman_theme_options['portfolio_viewstyle']); // Global view
	...
	if( isset($boldman_theme_options['show_topbar']) && $boldman_theme_options['show_topbar']==true ){
		$bodyClass[] = sanitize_html_class('themetechmount-topbar-yes');
	} else {
		$bodyClass[] = sanitize_html_class('themetechmount-topbar-no');
	}
	
	if( isset($boldman_theme_options['footer_cta_box']) && $boldman_theme_options['footer_cta_box']==true ){
		$bodyClass[] = sanitize_html_class('themetechmount-footer-cta-yes');
	}
	
	// Boxed / Wide
	if( isset($boldman_theme_options['layout']) && trim($boldman_theme_options['layout'])!='' ){
		if( $boldman_theme_options['layout']=='boxed' || $boldman_theme_options['layout']=='framed' || $boldman_theme_options['layout']=='rounded' ){
			$bodyClass[] = 'themetechmount-boxed';
		}
		if( $boldman_theme_options['layout']=='framed' || $boldman_theme_options['layout']=='rounded' ){
			$bodyClass[] = 'themetechmount-boxed-'.sanitize_html_class($boldman_theme_options['layout']);
		}
		
		$bodyClass[] = sanitize_html_class( 'themetechmount-'.trim($boldman_theme_options['layout']));
		if( $boldman_theme_options['layout']=='fullwide' ){
			if( isset($boldman_theme_options['full_wide_elements']['content']) && $boldman_theme_options['full_wide_elements']['content']=='1' ){
	...
	if( isset($boldman_theme_options['logoimg_retina']['url']) && $boldman_theme_options['logoimg_retina']['url']!=''){
	...
	$headerstyle_global = '';
	$headerstyle_page   = '';
	if( isset($boldman_theme_options['headerstyle']) && trim($boldman_theme_options['headerstyle'])!='' ){
		$headerstyle_global = sanitize_html_class($boldman_theme_options['headerstyle']);
	}
	if( is_page() ){
	...
	// One Page website
	if( isset($boldman_theme_options['one_page_site']) && $boldman_theme_options['one_page_site']==true ){
 ...
 */
if( !function_exists('themetechmount_add_cat_span') ){
function themetechmount_add_cat_span($catg) {
...
add_filter('wp_list_categories', 'themetechmount_add_cat_span');
