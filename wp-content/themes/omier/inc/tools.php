	...
	if( !empty($post_type) ){
		...
		if( !empty( $social_services ) && is_array( $social_services ) ){	
			foreach( $social_services as $social ){
				...
				<a href="javascript:void(0)" onClick="TMSocialWindow=window.open(\''. esc_url($link) .'\',\'TMSocialWindow\',width=600,height=100); return false;"><i class="tm-boldman-icon-'. sanitize_html_class($social) .'"></i></a>
				</li>';
...
function themetechmount_pagination( $wp_query_data=false ){
	...
	$paginateLinks = paginate_links( array(
		...
		'type'      => 'array',
		'prev_text' => '<i class="tm-boldman-icon-arrow-left"></i> <span class="tm-hide tm-pagination-text tm-pagination-text-prev">' . esc_attr__( 'Previous page', 'boldman' ) . '</span>',
		'next_text' => '<span class="tm-hide tm-pagination-text tm-pagination-text-next">' . esc_attr__( 'Next page', 'boldman' ) . '</span> <i class="tm-boldman-icon-arrow-right"></i>',
	) );
...
function themetechmount_get_option( $option, $inner_option='' ){
	...
	if( !is_array($boldman_theme_options) ){
		
		if( is_multisite() ){
			$boldman_theme_options = get_site_option('boldman_theme_options');
		} else {
	...
	if( isset($boldman_theme_options[$option]) ){
		$return = $boldman_theme_options[$option];
	} else {
...
function themetechmount_get_all_registered_sidebars(){
	...
	$return = array( '' => esc_attr__('Default', 'boldman') );
...
/**
 * Custom template tags for Boldman
 *
...
if ( ! function_exists( 'boldman_comment_nav' ) ) :
/**
 * Display navigation to next/previous comments when applicable.
 *
 * @since Boldman 1.0
 */
function boldman_comment_nav() {
	// Are there comments to navigate through?
	if ( get_comment_pages_count() > 1 && get_option( 'page_comments' ) ) :
	?>
	<nav class="navigation comment-navigation">
		<h2 class="screen-reader-text"><?php esc_attr_e( 'Comment navigation', 'boldman' ); ?></h2>
		<div class="nav-links">
			<?php
				if ( $prev_link = get_previous_comments_link( esc_attr__( 'Older Comments', 'boldman' ) ) ) :
					printf( '<div class="nav-previous">%s</div>', $prev_link );
				endif;

				if ( $next_link = get_next_comments_link( esc_attr__( 'Newer Comments', 'boldman' ) ) ) :
...
if ( ! function_exists( 'boldman_entry_meta' ) ) :
/**
 * Prints HTML with meta information for the categories, tags.
 *
 * @since Boldman 1.0
 */
function boldman_entry_meta( $metafor="blogbox" ) {
			...
			switch( $meta_id ){
				...
				case 'date':
					...
					if ( in_array( get_post_type(), array( 'post', 'attachment' ) ) ) {
						...
						$return .= sprintf( '<span class="tm-meta-line posted-on"><i class="fa fa-calendar"></i> <span class="screen-reader-text tm-hide">%1$s </span><a href="%2$s" rel="bookmark">%3$s</a></span>',
							esc_attr_x( 'Posted on', 'Used before publish date.', 'boldman' ),
				...
				case 'author':
					...
					if ( 'post' === get_post_type() ) {
						...
						$return .= sprintf( '<span class="tm-meta-line byline">' .get_avatar( get_the_author_meta( 'ID' ), 32 ) . '<span class="author vcard"><span class="screen-reader-text tm-hide">%1$s </span>%2$s</span></span>',
							esc_attr_x( 'Author', 'Used before post author name.', 'boldman' ),
							$author
						);
					}
				...
				case 'comment':
					...
					if ( ! post_password_required() && ( comments_open() || get_comments_number() ) ) {
						$return .= '<span class="tm-meta-line comments-link"><i class="tm-boldman-icon-comment-3"></i> ';
						ob_start();
						comments_popup_link( esc_attr__( '0 Comments', 'boldman' ) );
				...
				case 'cat':
					...
					if ( !empty($categories_list) ) {
						...
						$return .= sprintf( '<span class="tm-meta-line cat-links"><i class="tm-boldman-icon-folder"></i> <span class="screen-reader-text tm-hide">%1$s </span>%2$s</span>',
							esc_attr_x( 'Categories', 'Used before category names.', 'boldman' ),
							$categories_list
						);
					}
				...
				case 'tag':
					$tags_list = get_the_tag_list( '', esc_attr_x( ', ', 'Used between list items, there is a space after the comma.', 'boldman' ) );
					if ( !empty($tags_list) ) {
						if( $tag_link!=true ){
						...
						$return .= sprintf( '<span class="tm-meta-line tags-links"><i class="fa fa-tag"></i> <span class="screen-reader-text tm-hide">%1$s </span>%2$s</span>',
							esc_attr_x( 'Tags', 'Used before tag names.', 'boldman' ),
							$tags_list
						);
					}
 ...
 */
if( ! function_exists('boldman_categorized_blog') ){
function boldman_categorized_blog() {
	if ( false === ( $all_the_cool_cats = get_transient( 'boldman_categories' ) ) ) {
		...
		set_transient( 'boldman_categories', $all_the_cool_cats );
	}

	if ( $all_the_cool_cats > 1 ) {
		// This blog has more than 1 category so boldman_categorized_blog should return true.
		return true;
	} else {
		// This blog has only 1 category so boldman_categorized_blog should return false.
		return false;
	}
}
}
...
function themetechmount_blogbox_comment_count( $echo = false ) {
	...
	if( comments_open() ){
		...
		$return  .= '<div class="themetechmount-blogbox-comment"><i class="tm-boldman-icon-comment"></i> '. $comments .'</div>';
	}
	return $return;
}
}
	...
	if( !empty($type) ){
		...
		if( !empty($return) ){
			...
			switch( $type ){
				...
				case 'phone':
					$return = '<div class="tm-team-list-title"><i class="tm-boldman-icon-phone"></i> '. esc_attr__('Phone','boldman') .' :</div>
						<div class="tm-team-list-value"><a href="tel:' . $return . '">' . $return . '</a></div>';
					break;
					
				case 'email':
					$return = '<div class="tm-team-list-title"><i class="tm-boldman-icon-mail"></i> '. esc_attr__('Email','boldman') .' :</div>
				...
				case 'website':
					...
					$return = '<div class="tm-team-list-title"><i class="tm-boldman-icon-world"></i> '. esc_attr__('Website','boldman') .' :</div>
...
function themetechmount_create_icon_from_data( $data, $i_tag_only=false ){
	...
	if( isset($data['library']) && isset($data['library_fontawesome']) && isset($data['library_linecons']) && isset($data['library_themify']) ){
		...
		$library_kw_boldman = ( !empty($data['library_kw_boldman']) ) ? $data['library_kw_boldman'] : '';
		...
		if( !empty(${'library_'.$library}) && ${'library_'.$library}!='empty' ){
			...
			$return = do_shortcode('[tm-icon type="'.$library.'" icon_kw_boldman="'.$library_kw_boldman.'" icon_fontawesome="'.$library_fontawesome.'" icon_linecons="'.$library_linecons.'" icon_themify="'.$library_themify.'"]');
...
function themetechmount_portfolio_next_prev_btn() {
	...
	if( is_singular('tm_portfolio') ){
		...
		$return = get_the_post_navigation( array(
			'next_text' => '<span class="meta-nav" aria-hidden="true">' . esc_attr__( 'Next', 'boldman' ) . '</span> ' .
				'<span class="screen-reader-text tm-hide">' . esc_attr__( 'Next post:', 'boldman' ) . '</span> ' .
				'<span class="post-title tm-hide">%title</span>',
			'prev_text' => '<span class="meta-nav" aria-hidden="true">' . esc_attr__( 'Previous', 'boldman' ) . '</span> ' .
				'<span class="screen-reader-text tm-hide">' . esc_attr__( 'Previous post:', 'boldman' ) . '</span> ' .
				'<span class="post-title tm-hide">%title</span>',
		) );
...
function themetechmount_portfolio_detailsbox(){
	...
	if( isset($pf_details_line) && is_array($pf_details_line) && count($pf_details_line)>0 ){
		foreach( $pf_details_line as $key=>$val ){
			...
			if( !empty($val['pf_details_line_title']) ){ $row_title = sprintf( esc_attr__('%s ', 'boldman'), $val['pf_details_line_title'] ); }
...
function themetechmount_portfoliobox_media_link( $default_icon='tm-boldman-icon-search', $video_icon='tm-boldman-icon-video', $audio_icon='tm-boldman-icon-music-alt', $slider_icon='tm-boldman-icon-gallery-1' ){

	$icon_code='<i class="tm-boldman-icon-plus-1"></i>';
...
function themetechmount_gallerystyle_portfoliobox_media_link( $default_icon='tm-boldman-icon-search', $video_icon='tm-boldman-icon-video', $audio_icon='tm-boldman-icon-music-alt', $slider_icon='tm-boldman-icon-gallery-1' ){

	$icon_code='<i class="tm-boldman-icon-search"></i>';
	if( !empty($default_icon) ){
		$icon_code='<i class="' . $default_icon . '"></i>';
	}
...
function themetechmount_get_post_format_icon( $post_id='' ){
	...
	if( $post_type=='post' ){
		...
		$return = '<i class="tm-boldman-icon-' . $icon_class . '"></i>';
...
function themetechmount_get_boxes( $cpt='blog', $vars=array() ){
	...
	$allword  = (!empty($vars['allword'])) ? $vars['allword'] : esc_attr__('All', 'boldman');
...
function themetechmount_global_portfolio_template_list( $for_vc=false ){
	$return = array(
		'overlay'			=> esc_attr__('Content overlay on image view', 'boldman'),
		'top-image'			=> esc_attr__('Top Image and Bottom Content view', 'boldman'),
		'overlay-style2'    => esc_attr__('Content overlay on image style2', 'boldman'),
	);
...
function themetechmount_global_service_template_list( $for_vc=false ){
	$return = array(	
		'top-image'  => esc_attr__('Top Image and Bottom Content view', 'boldman'),
		'styleone'  => esc_attr__('Service Box - Style 1', 'boldman'),
...
function themetechmount_global_blog_template_list( $for_vc=false ){
	$return = array(
		"top-image"       => esc_attr__("Top image and bottom content (default)", "boldman"),
		"left-image"      => esc_attr__('Left image and right content ', "boldman"),
		"right-image"     => esc_attr__('Right image and left content ', "boldman"),
	);
...
function themetechmount_global_team_member_template_list( $for_vc=false ){
	$return = array(
		"overlay"  => esc_attr__("Top Image and Bottom Content", "boldman"),	
		"style3"  => esc_attr__("Overlay content on Image", "boldman"),		
	);
...
function themetechmount_global_testimonial_template_list( $for_vc=false ){
	$return = array(
		'style-1'    => esc_attr__('Testimonial Style 1', 'boldman'),
		'style-2' => esc_attr__('Testimonial Style 2', 'boldman'),
		'style-3' => esc_attr__('Testimonial Style 3', 'boldman'),
	);
...
function themetechmount_global_client_template_list( $for_vc=false ){
	$return = array(
		'simple-logo'    => esc_attr__('Simple Logo view', 'boldman'),
		'separator-logo' => esc_attr__('Logo with Separator view', 'boldman'),
		'boxed-logo'     => esc_attr__('Boxed view', 'boldman'),
	);
...
function themetechmount_blogbox_readmore_text(){
	...
	$return             = esc_attr__('Read More', 'boldman');
...
function themetechmount_servicebox_readmore_text(){
	...
	$return             = esc_attr__('More Details', 'boldman');
...
function themetechmount_servicebox_icon( $post_id='' ){
	...
	$box_icon = get_post_meta( get_the_ID(), 'themetechmount_tmservice_icon', true);
		...
		if( !empty($box_icon['tm_serviceicon']) ){	
			$return .= '<div class="tm-service-iconbox"><span class="tm-service-icon">' . themetechmount_create_icon_from_data( $box_icon['tm_serviceicon'], true ) . '</span></div>';
		}
	return $return;
}
}
...
function themetechmount_entry_icon() {
	...
	$iconCode .= '<i class="tm-boldman-icon-'.$icon.'"></i>';
...
function themetechmount_blogbox_single_meta( $option='date', $args=array() ) {
	...
	switch($option){
		...
		case 'category' :
			...
			$categories_list = get_the_category_list( esc_attr_x( ', ', 'Used between list items, there is a space after the comma.', 'boldman' ) );
			if ( !empty($categories_list) ) {
...
function themetechmount_titlebar_title(){
	...
	if( is_singular() || is_home() ){  // single page, single post and single cpt
		...
		if( is_home() ){
			...
			$title = esc_attr__( 'Blog', 'boldman' );  // Setting for Titlebar title
		}
		...
		$adv_tbar_catarc = ( !empty($adv_tbar_catarc) ) ? esc_attr($adv_tbar_catarc.' %s') : esc_attr__('Category Archives: %s', 'boldman') ;
		$title = sprintf(
			$adv_tbar_catarc,
		...
		$adv_tbar_tagarc = ( !empty( $adv_tbar_tagarc ) ) ? esc_attr($adv_tbar_tagarc.' %s') : esc_attr__('Tag Archives: %s','boldman') ;
		$title = sprintf(
			$adv_tbar_tagarc,
		...
		if( is_tax('tm_team_group') || is_tax('tm_portfolio_category') ){
			...
			global $wp_query;
			$adv_tbar_postclassified = ( !empty($adv_tbar_postclassified) ) ? esc_attr($adv_tbar_postclassified.' %s') : esc_attr__('Posts classified under: %s', 'boldman') ;
		...
		$adv_tbar_authorarc = ( !empty( $adv_tbar_authorarc ) ) ? esc_attr($adv_tbar_authorarc.' %s') : esc_attr__('Author Archives: %s', 'boldman');
		...
		$title    = sprintf( esc_attr__( 'Search Results for %s', 'boldman' ), '<span class="tm-titlebar-heading tm-tbar-search-title">' . get_search_query() . '</span>' );

			
	} else if( is_404() ){ // 404
		$error404_big_text = themetechmount_get_option('error404_big_text');
		$title             = sprintf( esc_attr__( '%s ', 'boldman' ), $error404_big_text );
		...
		if( function_exists('tribe_is_month') && tribe_is_month() && !is_tax() ) { // The Main Calendar Page
			...
			global $wp_query;
			$title = esc_attr__( 'Events on: ', 'boldman' ). date('F j, Y', strtotime($wp_query->query_vars['eventDate']));
		...
		// BBPress section
			...
			$title = esc_attr__( 'Forum', 'boldman' );
		} else if( is_post_type_archive() ){
			$title = post_type_archive_title('', false);
		} else if ( is_day() ){
			$title = sprintf( esc_attr__( 'Daily Archives: %s', 'boldman' ), '<span>' . get_the_date() . '</span>' );
		} elseif( is_month() ){
			$title = sprintf( esc_attr__( 'Monthly Archives: %s', 'boldman' ), '<span>' . get_the_date( esc_attr_x( 'F Y', 'monthly archives date format', 'boldman' ) ) . '</span>' );
		} elseif( is_year() ){
			$title = sprintf( esc_attr__( 'Yearly Archives: %s', 'boldman' ), '<span>' . get_the_date( esc_attr_x( 'Y', 'yearly archives date format', 'boldman' ) ) . '</span>' );
		} else {
			if( function_exists('is_bbpress') && is_bbpress() ) {
				$title = esc_attr__( 'Forum', 'boldman' );
			} else {
				$title = esc_attr__( 'Archives', 'boldman' );
			}
		};			
	...
	if( themetechmount_fbar_show()==true){
		...
		<span class="themetechmount-fbar-btn ' . themetechmount_sanitize_html_classes(themetechmount_fbar_btn_classes()) . '">
			<a href="#" class="themetechmount-fbar-btn-link">
				...
				<span class="tm-hide">' . esc_attr__('Open', 'boldman') . '</span>
			</a>
		</span>';
	}
...
function themetechmount_header_links(){
	...
	if( function_exists('is_woocommerce') && $wc_header_icon==true ){
		...
		$return  .= '<span class="tm-header-icon tm-header-wc-cart-link"><a href="' . $cart_url . '"><i class="tm-boldman-icon-basket"></i><span class="number-cart">' . esc_attr($count) . '</span></a></span>';
	}
	...
	if( $header_search==true ){
		$class   = '';
		$return .= '<div class="tm-header-icon tm-header-search-link"><a href="#"><i class="tm-boldman-icon-search"></i></a><div class="tm-search-overlay"><form method="get" class="tm-site-searchform" action=" ' . esc_url( home_url() ) . '"> <div class="w-search-form-h"><div class="w-search-form-row"><div class="w-search-input"><input type="search" class="field searchform-s" name="s" placeholder="' . esc_attr($search_input) . '" /><button type="submit"><span class="tm-boldman-icon-search"></span></button></div></div></div></form></div></div>';
	}
...
/**
 *  infostack header button
 */
if( !function_exists('themetechmount_infoheader_button') ){
function themetechmount_infoheader_button(){
	$return		 = '';
	$info_button    = themetechmount_get_option('infostack_phone_text');

	if( !empty($info_button) ){
		$return		= '<div class="ttm-custombutton">'.do_shortcode( $info_button ).'</div>';
	}
...
function themetechmount_footer_row_class( $row='first' ){
	...
	// BG color
	if( !empty($boldman_theme_options[$row.'_footer_bg_color']) ){
		$class .= ' tm-bg tm-bgcolor-'.sanitize_html_class($boldman_theme_options[$row.'_footer_bg_color']);
	}
	
	// Text color
	if( !empty($boldman_theme_options[$row.'_footer_text_color']) ){
		$class .= ' tm-textcolor-'.sanitize_html_class($boldman_theme_options[$row.'_footer_text_color']);
	}
	
	// If bg image is there
	if( !empty($boldman_theme_options[$row.'_footer_bg_all']['image']) ){
	...
	// border type
	if( !empty($boldman_theme_options[$row.'_footer_bordertype']) ){
		$class .= ' tm-bordertype-'.sanitize_html_class($boldman_theme_options[$row.'_footer_bordertype']);
	}
	
	//widget border color
	if( !empty($boldman_theme_options[$row.'_footer_widget_bordercolor']) ){
		$class .= ' tm-widgetbordercolor-'.sanitize_html_class($boldman_theme_options[$row.'_footer_widget_bordercolor']);
	}
...
function themetechmount_box_team_social_links(){
	...
	if( !empty($data) && is_array($data) && count($data)>0 ){
		...
		foreach($data as $social ){
			...
			$return .= '<li><a href="'. $social['social_icons_list_link'] .'" target="_blank" data-tooltip="' . $social_name . '"><i class="tm-boldman-icon-'. $social['social_icons_list_icon'] .'"></i><span class="tm-hide">'. $social_name .'</span></a></li>';
		}
 ...
 */
if( !function_exists('boldman_addhttp') ){
	function boldman_addhttp($url){
		if (!preg_match("~^(?:f|ht)tps?://~i", $url)){
			$url = "http://" . $url;
		}
		return $url;
...
function themetechmount_testimonial_featured_image($size='thumbnail'){
	...
	$return = ( !empty($featured_image) ) ? $featured_image : '<span class="themetechmount-icon-box"><i class="demo-icon tm-boldman-icon-quote-left"></i></span>';
...
function themetechmount_star_ratting(){
	...
	for ($i = 1; $i <= 5; $i++) {
		if( $star_ratings >= $i ){
			$return .= ' <i class="tm-boldman-icon-star-1 tm-active"></i> ';
		} else {
			$return .= ' <i class="tm-boldman-icon-star-empty-1"></i> ';
		}
	}
	return $return;
}
...
function themetechmount_revslider_array( $countonly=false ) {
	...
	if( $countonly==true ){
		...
		if( count($sliders)==0 ){
			$sliders[''] = esc_attr__('(No Slider Found)', 'boldman');
		}
		return $sliders;
	}
}
	...
	if ( function_exists('lsSliders') ) {
		...
		if( $countonly == true ){
			...
			if( count($slider_names) == 0 ){
				$slider_names[''] = esc_attr__('(No Slider Found)', 'boldman');
			}
			return $slider_names;
		}		
	}		
...
function themetechmount_single_get_sidebar_value(){
	...
	$sidebar = $boldman_theme_options['sidebar_post'];
	...
	if( is_page() || is_singular() ){
		...
		if( !empty($boldman_theme_options['sidebar_'.$cpt]) ){
			$sidebar = $boldman_theme_options['sidebar_'.$cpt];
		}
	...
	if( is_search() ){
		$sidebar = $boldman_theme_options['sidebar_search'];
	}
...
function themetechmount_event_description(){ 
	$return   = '';
	$readMore = esc_attr__('See Event', 'boldman') . '';
...
function themetechmount_event_readmore(){ 
	$return   = '';
	$readMore = esc_attr__('View Details', 'boldman') . ' <i class="kwicon-fa-angle-right"></i>';
...
function themetechmount_comment_row_template($comment, $args, $depth){
    ...
    <<?php echo esc_attr($tag); ?> <?php comment_class( empty( $args['has_children'] ) ? '' : 'parent' ) ?> id="comment-<?php comment_ID() ?>">
		...
		<?php if ( $comment->comment_approved == '0' ) : ?>
			 <em class="comment-awaiting-moderation"><?php esc_attr_e( 'Your comment is awaiting moderation.', 'boldman' ); ?></em>
			  <br />
		<?php endif; ?>
		<div class="comment-box">
			<div class="comment-meta commentmetadata">
				<?php printf( '<cite class="tm-comment-owner fn">%s</cite> <span class="says">' . esc_attr__('says:', 'boldman' ) . '</span>', get_comment_author_link() ); ?>
				
				<a href="<?php echo htmlspecialchars( get_comment_link( $comment->comment_ID ) ); ?>">
				<?php
				/* translators: 1: date, 2: time */
				printf( esc_attr__( '%1$s at %2$s', 'boldman' ), get_comment_date(),  get_comment_time() ); ?></a><?php edit_comment_link( esc_attr__( '(Edit)', 'boldman' ), '  ', '' );
				?>
			</div>			
...
function themetechmount_author_social_links(){
	...
	foreach( $all_socials as $social_class => $social_link ){
		if( !empty($social_link) ){
			$return .= '<li><a href="'. $social_link .'" target="_blank"><i class="tm-boldman-icon-'. $social_class .'"></i><span class="tm-hide">'. ucwords($social_class) .'</span></a></li>';
		}
	}
...
function themetechmount_search_results_box_title( $post_type='post' ){
	...
	$small_link = '<small><a href="'. esc_url(get_home_url()).'?s='.get_search_query().'" class="label label-default"><i class="tm-boldman-icon-angle-left"></i> '.esc_attr__('Back to results','boldman').'</a></small>';
	if( empty($_GET['post_type']) ){
		$small_link = '<small><a href="'. esc_url(get_home_url()).'?s='.get_search_query().'&post_type=' . $post_type . '" class="label label-default">'.esc_attr__('View more','boldman').'</a></small>';
	}
...
function themetechmount_search_form(){
	...
	$cptList = array(
		'any'            => esc_attr__('All selections', 'boldman'),
		'page'           => esc_attr__('Pages', 'boldman'),
		'post'           => esc_attr__('Blog posts', 'boldman'),
	);
	...
	<div class="tm-sresult-form-wrapper">
					...
					<div class="tm-sresult-form-bottom-w">
						...
						<div class="tm-search-main-box clearfix">
                            <div class="tm-search-text"><strong>' . esc_attr__('Search in:','boldman') . '</strong></div>
							<div class="tm-search-select-box">
								' . $dropdown . '
                              <div class="tm-sresult-form-sbtbtn-wrapper">
								<input class="tm-sresult-form-sbtbtn" type="submit" value="' . esc_attr__('Search now','boldman') . '" />
							  </div>
						  </div>
...
function themetechmount_footer_ctabox(){
	...
	if( !empty($boldman_theme_options['footer_cta_column_layout']) ){
	...
	if( $footer_cta_box==true ){
		...
		echo themetechmount_wp_kses('<div class="tm-footer-cta-wrapper container"><div class="row tm-footer-cta-inner tm-bg tm-bgcolor-'.themetechmount_sanitize_html_classes($footer_cta_bg_color).' tm-textcolor-'.themetechmount_sanitize_html_classes($footer_cta_text_color).'">');
		...
		echo themetechmount_wp_kses('</div></div>');
	}	
}
}
...
function themetechmount_entry_date( $echo = true ) {
	if ( has_post_format( array( 'chat', 'status' ) ) ){
		$format_prefix = esc_attr_x( '%1$s on %2$s', '1: post format name. 2: date', 'boldman' );
	} else {
		$format_prefix = '%2$s';
	}
...
if ( ! function_exists( 'boldman_posttime_meta' ) ) :
function boldman_posttime_meta( $metafor="blogbox" ) {
...
function themetechmount_get_socialbar_links( $tooltip_position='top' , $tooltip='yes' ){
	...
	$return = '';
	if( !empty($boldman_theme_options['social_icons_list']) ){
		foreach( $boldman_theme_options['social_icons_list'] as $socialicon ){
			...
			$return .= '<li class="tm-social-' . $social_id . ' tm-socialbox-i-wrapper"><a class="tm-socialbox-icon-link tm-socialbox-icon-link-' . $social_id . ' ' . sanitize_html_class($class) . '" target="_blank" href="' . $href . '"><span class="frame-hover"></span><i class="tm-boldman-icon-' . $social_id . '"></i><span class="social_name">'.$social_name.'</span></a></li>' . "\n";
		}
	}
	foreach( $socialArray as $key=>$value ){
		
		if( $key == 'rss' ){
			if( isset($boldman_theme_options['rss']) && $boldman_theme_options['rss']=='1' ){
			...
			if( isset($boldman_theme_options[$key]) && trim($boldman_theme_options[$key])!='' ){
				$return .= '<li class="'.$key.' "><a target="_blank" href="'.esc_url($boldman_theme_options[$key]).'" data-tooltip="'.$value[1].'"><i class="tm-social-icon-'.$value[0].'"></i></a></li>';
			}
		}
	}
	if( $return!='' ){
