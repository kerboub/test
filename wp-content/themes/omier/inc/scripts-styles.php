...
function themetechmount_login_stylesheet() {
    wp_enqueue_style( 'boldman-login-style', get_template_directory_uri() . '/style-login.min.css' );
}
}
...
function themetechmount_min_css(){
	...
	if(!is_admin()) {
		if( isset($boldman_theme_options['minify']) && $boldman_theme_options['minify']==true ){
			define('TM_MIN', '.min');
		} else {
			define('TM_MIN', '');
		}
 ...
 * @return void
 */
if( !function_exists('boldman_scripts_styles') ){
function boldman_scripts_styles() {
	global $boldman_theme_options;
	/*
	...
	if( isset($boldman_theme_options['pagetranslation']) && $boldman_theme_options['pagetranslation']!='no'){
	...
	// Sticky
	if( !empty($boldman_theme_options['sticky_header']) && $boldman_theme_options['sticky_header']=='y' ){
	...
	wp_enqueue_style( 'tm-boldman-icons', get_template_directory_uri() . '/assets/tm-boldman-icons/css/tm-boldman-icons.css' );
	
	// Extra icons related to different businesses
	wp_enqueue_style( 'themetechmount-boldman-extra-icons', get_template_directory_uri() . '/assets/themetechmount-boldman-extra-icons/font/flaticon.css' );
...
add_action( 'wp_enqueue_scripts', 'boldman_scripts_styles', 10 );

if( !function_exists('boldman_scripts_styles_14') ){
function boldman_scripts_styles_14() {
	...
	if ( file_exists( ABSPATH . 'wp-content/plugins/js_composer/assets/css/js_composer_tta.min.css') ) {
		wp_enqueue_style( 'vc-tta-style',  plugins_url() . '/js_composer/assets/css/js_composer_tta.min.css' );
	}
	
	wp_enqueue_style( 'boldman-base-style', get_template_directory_uri() . '/css/base'.TM_MIN.'.css', array('bootstrap','bootstrap-theme') );
}
}
add_action( 'wp_enqueue_scripts', 'boldman_scripts_styles_14', 14 );

if( !function_exists('boldman_scripts_styles_15') ){
function boldman_scripts_styles_15() {
	global $boldman_theme_options;
	$min = TM_MIN;
	if( is_child_theme() ){
		$min = '';
	...
	if( defined( 'WPB_VC_VERSION' ) ){
		wp_enqueue_style( 'boldman-main-style', get_template_directory_uri() . '/css/main'.TM_MIN.'.css' , array('js_composer_front') );
		wp_register_style( 'boldman-dark', get_template_directory_uri() . '/css/dark'.TM_MIN.'.css' , array('js_composer_front', 'boldman-main-style') );  // Dark
	} else {
		wp_enqueue_style( 'boldman-main-style', get_template_directory_uri() . '/css/main'.TM_MIN.'.css' );
		wp_register_style( 'boldman-dark', get_template_directory_uri() . '/css/dark'.TM_MIN.'.css' , array( 'boldman-main-style') );  // Dark
	}
	
	// Service box animation css
	wp_enqueue_style( 'boldman-servicebox-animation', get_template_directory_uri() . '/css/servicebox-animation'.TM_MIN.'.css', array( 'boldman-main-style') );
	
	// Load dark.css if dark layout
	if( isset($boldman_theme_options['inner_background']['background-color']) && trim($boldman_theme_options['inner_background']['background-color'])!='' && themetechmount_check_dark_color($boldman_theme_options['inner_background']['background-color']) ){
		wp_enqueue_style('boldman-dark');
	}
}
}
add_action( 'wp_enqueue_scripts', 'boldman_scripts_styles_15', 15 );

if( !function_exists('boldman_scripts_styles_17') ){
function boldman_scripts_styles_17() {
	// Responsive
	global $boldman_theme_options;
	
	// Responsive CSS
	wp_enqueue_style( 'boldman-responsive-style', get_template_directory_uri() . '/css/responsive'.TM_MIN.'.css' );
	
	// Loads JavaScript file with functionality specific to Boldman.
	if ( wp_script_is( 'wpb_composer_front_js', 'registered' ) ) {
		wp_enqueue_script( 'boldman-script', get_template_directory_uri() . '/js/functions'.TM_MIN.'.js', array( 'jquery', 'wpb_composer_front_js' ), '1.0', true );
	} else {
		wp_enqueue_script( 'boldman-script', get_template_directory_uri() . '/js/functions'.TM_MIN.'.js', array( 'jquery' ), '1.0', true );
	}
	
	wp_enqueue_style( 'boldman-last-checkpoint', get_template_directory_uri() . '/css/boldman-last-checkpoint.min.css' );		
}
}
add_action( 'wp_enqueue_scripts', 'boldman_scripts_styles_17', 17 );