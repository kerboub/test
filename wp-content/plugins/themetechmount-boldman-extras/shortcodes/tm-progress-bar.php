	...
	if( function_exists('vc_map') ){
		...
		foreach ( $graph_lines_data as $line ) {
			...
			if( !empty($line['add_icon']) && $line['add_icon']=='true' ){
				...
				$return .= do_shortcode('[tm-icon type="' . $line['i_type'] . '" color="' . $icon_color . '" icon_linecons="' . $line['i_icon_linecons'] . '" icon_themify="' . $line['i_icon_themify'] . '" icon_fontawesome="' . $line['i_icon_fontawesome'] . '" icon_kw_boldman="' . $line['i_icon_kw_boldman'] . '" ]');
				$return .= '</div>';
			}
			...
			$return .= '<div class="vc_general vc_single_bar' . ( ( isset( $line['color'] ) && 'custom' !== $line['color'] ) ? ' ' . sanitize_html_class('vc_progress-bar-color-'.$line['color']) : '' ) . '">';
				...
				$return .= themetechmount_wp_kses($unit);
