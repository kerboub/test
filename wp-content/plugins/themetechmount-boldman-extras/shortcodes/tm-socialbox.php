		...
		if( is_array($socialdata) && count($socialdata)>0 ){
			foreach( $socialdata as $data ){
				if( !empty($data->servicename) ){
					...
					$return .= '<a class="tm-socialbox-icon-link tm-socialbox-icon-link-'.$data->servicename.'" target="_blank" href="'.$servicelink.'" data-tooltip="'.$servicename.'"><i class="tm-boldman-icon-'.$data->servicename.'"></i></a>';
					$return .= '</div>';
