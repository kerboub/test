			...
			switch( $meta_id ){
				...
				case 'date':
					...
					if ( in_array( get_post_type(), array( 'post', 'attachment' ) ) ) {
						...
						$return .= sprintf( '<span class="tm-meta-line posted-on"><i class="tm-boldman-icon-clock"></i> <span class="screen-reader-text tm-hide">%1$s </span><a href="%2$s" rel="bookmark">%3$s</a></span>',
				...
				case 'author':
					...
					if ( 'post' === get_post_type() ) {
						...
						$return .= sprintf( '<span class="tm-meta-line byline"><i class="tm-boldman-icon-user"></i> <span class="author vcard"><span class="screen-reader-text tm-hide">%1$s </span>%2$s</span></span>',
				...
				case 'comment':
					...
					if ( ! post_password_required() && ( comments_open() || get_comments_number() ) ) {
						$return .= '<span class="tm-meta-line comments-link"><i class="tm-boldman-icon-comments-smiley"></i> ';
						ob_start();
						comments_popup_link( esc_attr__( 'Leave a comment', 'boldman' ) );
				...
				case 'cat':
					...
					if ( !empty($categories_list) ) {
						...
						$return .= sprintf( '<span class="tm-meta-line cat-links"><i class="tm-boldman-icon-category"></i> <span class="screen-reader-text tm-hide">%1$s </span>%2$s</span>',
				...
				case 'tag':
					...
					if ( !empty($tags_list) ) {
						...
						$return .= sprintf( '<span class="tm-meta-line tags-links"><i class="tm-boldman-icon-tag"></i> <span class="screen-reader-text tm-hide" tm-hide>%1$s </span>%2$s</span>',
	...
	if( !empty($type) ){
		...
		if( !empty($return) ){
			...
			switch( $type ){
				...
				case 'phone':
					$return = '<div class="tm-team-list-title tm-skincolor"><i class="tm-boldman-icon-phone"></i> '. esc_attr__('Phone','boldman') .'</div>
						<div class="tm-team-list-value"><a href="tel:' . $return . '">' . $return . '</a></div>';
					break;
					
				case 'email':
					$return = '<div class="tm-team-list-title tm-skincolor"><i class="tm-boldman-icon-mail"></i> '. esc_attr__('Email','boldman') .'</div>
				...
				case 'website':
					...
					$return = '<div class="tm-team-list-title tm-skincolor"><i class="tm-boldman-icon-world"></i> '. esc_attr__('Website','boldman') .'</div>
...
function themetechmount_portfolio_next_prev_btn() {
	...
	if( is_singular('tm_portfolio') ){
		...
		$return = get_the_post_navigation( array(
			'next_text' => '<span class="meta-nav tm-hide" aria-hidden="true">' . __( 'Next', 'boldman' ) . '</span> ' .
				'<span class="screen-reader-text tm-hide">' . __( 'Next post:', 'boldman' ) . '</span> ' .
				'<span class="post-title tm-hide">%title</span>',
			'prev_text' => '<span class="meta-nav tm-hide" aria-hidden="true">' . __( 'Previous', 'boldman' ) . '</span> ' .
				'<span class="screen-reader-text tm-hide">' . __( 'Previous post:', 'boldman' ) . '</span> ' .
				'<span class="post-title tm-hide">%title</span>',
		) );
...
function themetechmount_portfolio_detailsbox(){
	...
	if( isset($pf_details_line) && is_array($pf_details_line) && count($pf_details_line)>0 ){
		foreach( $pf_details_line as $key=>$val ){
			...
			if( !empty($val['pf_details_line_title']) ){ $row_title = sprintf( __('%s', 'boldman'), $val['pf_details_line_title'] ); }
...
function themetechmount_global_blog_template_list( $for_vc=false ){
	$return = array(
		...
		"content-overlay" => esc_attr__('Content overlay on image', "boldman"),
	);
...
function themetechmount_global_team_member_template_list( $for_vc=false ){
	$return = array(
		"overlay"    => esc_attr__("Overlay content on Image (Default)", "boldman"),
		"left-image" => esc_attr__("Left Image and Right Content", "boldman"),
	);
...
function themetechmount_global_client_template_list( $for_vc=false ){
	$return = array(
		...
		//'logo-with-title' => esc_attr__('Logo with Title view', 'boldman'),
	);
...
function themetechmount_header_links(){
	...
	if( function_exists('is_woocommerce') && $wc_header_icon==true ){
		...
		$return  .= '<span class="tm-header-icon tm-header-wc-cart-link"><a href="' . $cart_url . '"><i class="tm-boldman-icon-cart"></i><span class="number-cart">&nbsp;</span></a></span>';
	}
	...
	if( $header_search==true ){
		$class   = '';
		$return .= '<span class="tm-header-icon tm-header-search-link"><a href="javascript:void(0);"><i class="tm-boldman-icon-search"></i></a></span>';
	}
...
function themetechmount_box_team_social_links(){
	...
	if( !empty($data) && is_array($data) && count($data)>0 ){
		...
		foreach($data as $social ){
			...
			$return .= '<li><a href="'. $social['social_icons_list_link'] .'" target="_blank"><i class="tm-boldman-icon-'. $social['social_icons_list_icon'] .'"></i><span class="tm-hide">'. $social_name .'</span></a></li>';
		}
...
function themetechmount_event_description(){ 
	$return   = '';
	$readMore = esc_attr__('See Event', 'boldman') . ' <i class="kwicon-fa-angle-right"></i>';
...
function themetechmount_comment_row_template($comment, $args, $depth){
    ...
    <div class="comment-meta commentmetadata">
		<?php printf( esc_attr__( '<cite class="tm-comment-owner fn">%s</cite> <span class="says">says:</span>', 'boldman' ), get_comment_author_link() ); ?>
...
function themetechmount_search_results_box_title( $post_type='post' ){
	...
	if( !empty($singular_name) ){
		...
		$return .= '<div class="tm-sresults-title-w"><h2 class="tm-sresults-title">' . sprintf(
				esc_attr__('Search results for %s','boldman'),
