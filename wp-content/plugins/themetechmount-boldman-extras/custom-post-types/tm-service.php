...
function themetechmount_boldman_cpt_tm_service(){

	$boldman_theme_options = get_option('boldman_theme_options');
	
	$service_type_title          = ( !empty($boldman_theme_options['service_type_title']) ) ? $boldman_theme_options['service_type_title'] : 'Service' ;
	...
	$service_type_slug           = ( !empty($boldman_theme_options['service_type_slug']) ) ? $boldman_theme_options['service_type_slug'] : 'service' ;
	
	$service_cat_title          = ( !empty($boldman_theme_options['service_cat_title']) ) ? $boldman_theme_options['service_cat_title'] : 'Service Categories' ;
	$service_cat_title_singular = ( !empty($boldman_theme_options['service_cat_title_singular']) ) ? $boldman_theme_options['service_cat_title_singular'] : 'Service Category' ;
	$service_cat_slug           = ( !empty($boldman_theme_options['service_cat_slug']) ) ? $boldman_theme_options['service_cat_slug'] : 'service-category' ;
...
add_action( 'init', 'themetechmount_boldman_cpt_tm_service', 8 );
 ...
 *  Meta Boxes: Service
 */
if ( ! function_exists( 'themetechmount_boldman_tm_service_metabox_options' ) ) {
function themetechmount_boldman_tm_service_metabox_options( $options ) {
	...
	$list_array    = array();
	$options_array = array();
	if( isset($boldman_theme_options['service_details_line']) && is_array($boldman_theme_options['service_details_line']) && count( $boldman_theme_options['service_details_line'] ) > 0 ){
		foreach( $boldman_theme_options['service_details_line'] as $key=>$val ){
			...
			$option_array = array(
				...
				'type'       => 'text',
				'title'      => '<span class="tm-admin-service-list-icon"> <i class="'. $icon_class .'"></i></span> &nbsp; '. esc_attr__($val['service_details_line_title'], 'boldman'),
			);
	...
	$options[]    = array(
		...
		'title'     => esc_attr__('Boldman: Featured Image / Video / Slider', 'tmte'),
...
add_filter( 'cs_metabox_options', 'themetechmount_boldman_tm_service_metabox_options' );