...
function themetechmount_boldman_cpt_tm_team(){

	// Getting Options
	$boldman_theme_options = get_option('boldman_theme_options');
	
	$team_type_title          = ( !empty($boldman_theme_options['team_type_title']) ) ? $boldman_theme_options['team_type_title'] : 'Team Members' ;
	...
	$team_type_slug           = ( !empty($boldman_theme_options['team_type_slug']) ) ? $boldman_theme_options['team_type_slug'] : 'team-member' ;
	
	$team_group_title          = ( !empty($boldman_theme_options['team_group_title']) ) ? $boldman_theme_options['team_group_title'] : 'Team Groups' ;
	$team_group_title_singular = ( !empty($boldman_theme_options['team_group_title_singular']) ) ? $boldman_theme_options['team_group_title_singular'] : 'Team Group' ;
	$team_cat_slug           = ( !empty($boldman_theme_options['team_cat_slug']) ) ? $boldman_theme_options['team_cat_slug'] : 'team-group' ;
	...
	add_action('do_meta_boxes', 'themetechmount_boldman_tm_team_featured_image_box');
	function themetechmount_boldman_tm_team_featured_image_box() {
...
add_action( 'init', 'themetechmount_boldman_cpt_tm_team', 8 );
...
function themetechmount_tm_team_member_set_featured_image_column($columns) {
	...
	foreach( $columns as $key=>$val ){
		$new_columns[$key] = $val;
		if( $key=='title' ){
			$new_columns['themetechmount_featured_image'] = esc_attr__( 'Featured Image', 'boldman' );
		}
	}
	return $new_columns;
}
 ...
 *  Meta Box: Team
 */
if ( ! function_exists( 'themetechmounttmte_boldman_tm_team_metabox_options' ) ) {
function themetechmounttmte_boldman_tm_team_metabox_options( $options ) {
	...
	$team_extra_details_lines  = ( !empty($boldman_theme_options['team_extra_details_lines']) ) ? $boldman_theme_options['team_extra_details_lines'] : array() ;
	...
	if( is_array($team_extra_details_lines) && count($team_extra_details_lines) > 0 ){
		...
		foreach( $boldman_theme_options['team_extra_details_lines'] as $key=>$val ){
			...
			$this_array['title'] = '<span class="tm-admin-team-list-icon"> <i class="'. $icon_class .'"></i></span> &nbsp; '. esc_attr__($val['team_extra_details_line_title'], 'boldman');
	...
	$options[]    = array(
		...
		'title'     => sprintf( esc_attr__("Boldman: %s's Details", 'tmte'), $team_type_title_singular ),
	...
	$options[]    = array(
		...
		'title'     => sprintf( esc_attr__("Boldman: %s's Social Links", 'tmte'), $team_type_title_singular ),
		'post_type' => 'tm_team_member', // only here is important
		'context'   => 'normal',
		'priority'  => 'default',
		'sections'  => array(
			...
			array(
				...
				'fields' => array(
					array(
						...
						'info'            => esc_attr__('Add your social services here. Also you can reorder the Social Links as per your choice. Just drag and drop items to reorder and save settings', 'boldman'),
						'button_title'    => esc_attr__('Add New Social Links', 'boldman'),
						'accordion_title' => 'social_icons_list_icon',
						'fields'          => array(
							array(
								'id'            => 'social_icons_list_icon',
								'type'          => 'select',
								'title'         =>  esc_attr__('Social Sevice', 'boldman'),
								'options'  		=> array(
													'twitter'    => esc_attr__('Twitter', 'boldman' ),
													'youtube'    => esc_attr__('YouTube', 'boldman' ),
													'flickr'     => esc_attr__('Flickr', 'boldman' ),
													'facebook'   => esc_attr__('Facebook', 'boldman' ),
													'linkedin'   => esc_attr__('LinkedIn', 'boldman' ),
													'gplus'      => esc_attr__('Google+', 'boldman' ),
													'yelp'       => esc_attr__('Yelp', 'boldman' ),
													'dribbble'   => esc_attr__('Dribbble', 'boldman' ),
													'pinterest'  => esc_attr__('Pinterest', 'boldman' ),
													'podcast'    => esc_attr__('Podcast', 'boldman' ),
													'instagram'  => esc_attr__('Instagram', 'boldman' ),
													'xing'       => esc_attr__('Xing', 'boldman' ),
													'vimeo'      => esc_attr__('Vimeo', 'boldman' ),
													'vk'         => esc_attr__('VK', 'boldman' ),
													'houzz'      => esc_attr__('Houzz', 'boldman' ),
													'issuu'      => esc_attr__('Issuu', 'boldman' ),
													'google-drive' => esc_attr__('Google Drive', 'boldman' ),
													'rss'        => esc_attr__('RSS', 'boldman' ),
												),
								'class'         => 'chosen',
								'default'       => 'twitter',
								'after'  		=> '<div class="cs-text-muted"><br>'.__('Select Social service from here', 'boldman').'</div>',
							),
							array(
								'id'     		=> 'social_icons_list_link',
								'type'    		=> 'text',
								'title'   		=> esc_attr__('Link to Social service selected above', 'boldman'),
...
add_filter( 'cs_metabox_options', 'themetechmounttmte_boldman_tm_team_metabox_options' );
