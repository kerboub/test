...
function themetechmount_boldman_cpt_tm_portfolio(){
	...
	$pf_type_slug           = ( !empty($boldman_theme_options['pf_type_slug']) ) ? $boldman_theme_options['pf_type_slug'] : 'portfolio' ;
	...
	$pf_cat_slug           = ( !empty($boldman_theme_options['pf_cat_slug']) ) ? $boldman_theme_options['pf_cat_slug'] : 'portfolio-category' ;
...
add_action( 'init', 'themetechmount_boldman_cpt_tm_portfolio', 8 );
 ...
 */
if ( ! function_exists( 'themetechmount_boldman_tm_portfolio_metabox_options' ) ) {
function themetechmount_boldman_tm_portfolio_metabox_options( $options ) {
	...
	$list_array    = array();
	$options_array = array();
	if( isset($boldman_theme_options['pf_details_line']) && is_array($boldman_theme_options['pf_details_line']) && count( $boldman_theme_options['pf_details_line'] ) > 0 ){
		foreach( $boldman_theme_options['pf_details_line'] as $key=>$val ){
			...
			$option_array = array(
				...
				'type'       => 'text',
				'title'      => '<span class="tm-admin-pf-list-icon"> <i class="'. $icon_class .'"></i></span> &nbsp; '. esc_attr__($val['pf_details_line_title'], 'boldman'),
			);
	...
	$options[]    = array(
		...
		'title'     => sprintf( esc_attr__('Boldman: %s List Options', 'tmte'), $pf_type_title_singular ),
	...
	$options[]    = array(
		...
		'title'     => sprintf( esc_attr__('Boldman: %s View Style', 'tmte'), $pf_type_title_singular ),
		'post_type' => 'tm_portfolio', // only here is important
		'context'   => 'normal',
		'priority'  => 'default',
		'sections'  => array(
	...
	$options[]    = array(
		...
		'title'     => esc_attr__('Boldman: Portfolio Like Option','tmte'),
		'post_type' => 'tm_portfolio', // only here is important
		'context'   => 'normal',
		'priority'  => 'default',
		'sections'  => array(
...
add_filter( 'cs_metabox_options', 'themetechmount_boldman_tm_portfolio_metabox_options' );