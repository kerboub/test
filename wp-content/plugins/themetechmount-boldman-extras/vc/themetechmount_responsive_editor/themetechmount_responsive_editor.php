...
function themetechmount_responsive_editor_field( $settings, $value ) {
	...
	$responsive_sizes = array(
		'1200' => array(
			...
			esc_attr('Large', 'boldman'), // name
		),
		'991' => array(
			'dashicons-tablet', // icon class
			esc_attr('Medium', 'boldman'), // name
		),
		'767' => array(
			'dashicons-smartphone', // icon class
			esc_attr('Small', 'boldman'), // name
		),
		'custom' => array(
			'dashicons-editor-code', // icon class
			esc_attr('Custom', 'boldman'), // name
		),
	...
	foreach($responsive_sizes as $size=>$val){
		...
		if( !empty($settings['break_column_option']) && $settings['break_column_option']=='no' ){
			$option_break_column = '<div class="tm-responsive-editor-colbreak-w" style="display:none;"><label><input type="checkbox" value="colbreak_yes" name="' . $settings['param_name'] . '_'.$size.'_break_col" '.esc_attr($checked).' /> ' . esc_attr__('Break Column in this (and below) screen size.','boldman').'</label></div>';
		} else {
			$option_break_column = '<div class="tm-responsive-editor-colbreak-w"><label><input type="checkbox" value="colbreak_yes" name="' . $settings['param_name'] . '_'.$size.'_break_col" '.esc_attr($checked).' /> ' . esc_attr__('Break Column in this (and below) screen size.','boldman').'</label></div>';
		}
